...
try {
  ...
  const validV4Uuid = new Uint8Array([
    ...
    0x12, 0x34, 0x56, 0x78, 0x12, 0x34, 0x43, 0x21,
    0x88, 0x88, 0x12, 0x34, 0x56, 0x78, 0x90, 0xab,
    0x00 // Type
  ]);
  const result1 = testProtocolSniffer(validV4Uuid.buffer as ArrayBuffer);
  assert.strictEqual(result1, "neko", "Test 1 Failed: Valid UUID v4 should trigger Trojan/neko signature");
  ...
  const invalidV4Uuid = new Uint8Array([
    ...
    0x12, 0x34, 0x56, 0x78, 0x12, 0x34, 0x33, 0x21,
    0x88, 0x88, 0x12, 0x34, 0x56, 0x78, 0x90, 0xab,
    0x00 // Type
  ]);
