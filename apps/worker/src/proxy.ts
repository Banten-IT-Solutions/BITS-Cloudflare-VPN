import type { Proxy, SubQuery } from "@bits-vpn/shared";
...
export async function getCachedSub(env: Env, q: SubQuery): Promise<{ bytes: ArrayBuffer; contentType: string } | null> {
  const cacheKey = subCacheKey(q);
  const cached = await env.KV.get(cacheKey, "arrayBuffer");
  if (!cached) return null;
  return {
    bytes: cached,
    contentType: q.format === "raw" ? "text/plain" : "application/json",
  };
}

export async function setCachedSub(
  env: Env,
  q: SubQuery,
  buffer: ArrayBuffer,
): Promise<void> {
  await env.KV.put(subCacheKey(q), buffer, {
    expirationTtl: Math.floor(SUB_CACHE_TTL_MS / 1000),
  });
}