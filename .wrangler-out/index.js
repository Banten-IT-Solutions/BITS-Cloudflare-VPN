...
var v2 = "djJyYXk=";
var PORTS = [443, 80];
var PROTOCOLS = [atob(neko), atob(horse), atob(flash)];
var KV_PRX_URL = "https://raw.githubusercontent.com/bitscoid/BITS-Cloudflare-VPN/main/KV.json";
var PRX_BANK_URL = "https://raw.githubusercontent.com/bitscoid/BITS-Cloudflare-VPN/main/proxy.txt";
...
var SALT_A1 = atob("Vk1lc3MgSGVhZGVyIEFFQUQgS2V5X0xlbmd0aA==");
var SALT_A2 = atob("Vk1lc3MgSGVhZGVyIEFFQUQgTm9uY2VfTGVuZ3Ro");
var SALT_A3 = atob("Vk1lc3MgSGVhZGVyIEFFQUQgS2V5");
var SALT_A4 = atob("Vk1lc3MgSGVhZGVyIEFFQUQgTm9uY2U=");
var SALT_B1 = atob("QUVBRCBSZXNwIEhlYWRlciBMZW4gS2V5");
var SALT_B2 = atob("QUVBRCBSZXNwIEhlYWRlciBMZW4gSVY=");
var SALT_B3 = atob("QUVBRCBSZXNwIEhlYWRlciBLZXk=");
var SALT_B4 = atob("QUVBRCBSZXNwIEhlYWRlciBJVg==");
...
// src/core/relay.ts
var prxIP = "";
function setPrxIP(ip) {
  prxIP = ip;
}
__name(setPrxIP, "setPrxIP");
...
async function protocolSniffer(buffer) {
  ...
  if (buffer.byteLength >= 18) {
    const version2 = new Uint8Array(buffer.slice(0, 1))[0];
    if (version2 === 0) {
...
async function generateStreamResponseHeader(responseOptions, encKey, encIv) {
  try {
    const key = (await sha2562(encKey)).slice(0, 16);
    const iv = (await sha2562(encIv)).slice(0, 16);
...
__name(generateStreamResponseHeader, "generateStreamResponseHeader");
...
async function kdf(key, path) {
  async function hmacSha256(key2, data) {
    const hmacKey = await crypto.subtle.importKey("raw", key2, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
  ...
  __name(recursiveHash, "recursiveHash");
  const sha256Hash = /* @__PURE__ */ __name(async (data) => {
...
async function readStreamHeader(buffer) {
  try {
    ...
    let offset = 0;
    const version2 = view.getUint8(offset);
    offset += 1;
    if (version2 !== 1) {
      return { hasError: true, message: `Invalid protocol version: ${version2}` };
    }
...
__name(readStreamHeader, "readStreamHeader");
...
__name(readNekoHeader, "readNekoHeader");
...
async function checkPrxHealth(prxIP2, prxPort) {
  ...
  const timeoutMs = 5e3;
  try {
    const socket = connect({ hostname: prxIP2, port: Number(prxPort) });
    await Promise.race([
      socket.closed.catch((error3) => {
        throw error3;
      }),
...
function createApiRoutes() {
  ...
  app2.get("/sub", async (c) => {
    ...
    const fillerDomain = c.req.query("domain") || domain2;
    ...
    for (const prx of prxList) {
      ...
      uri.searchParams.set("host", domain2);
      for (const port of filterPort) {
        for (const protocol of protocols) {
          ...
          uri.searchParams.set("sni", port == 80 && protocol == atob("dm1lc3M=") ? "" : domain2);
  ...
  app2.get("/myip", async (c) => {
    ...
    return c.json(
      {
        ip: req.headers.get("cf-connecting-ipv6") || req.headers.get("cf-connecting-ip") || req.headers.get("x-real-ip"),
  ...
  app2.get("/proxies", async (c) => {
    ...
    const count3 = items.length;
    const pages = Math.ceil(count3 / limit);
    ...
    try {
      ...
      if (upgradeHeader === "websocket") {
        ...
        if (url.pathname.length == 3 || url.pathname.match(",")) {
          ...
          const kvPrx = await getKVPrxList(env2.KV_PRX_URL);
          const prxIP2 = kvPrx[prxKey][Math.floor(Math.random() * kvPrx[prxKey].length)];
          setPrxIP(prxIP2);
